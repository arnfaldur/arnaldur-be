...
a unit n-cube has a unit <abbr>volume</abbr> (put "n-dimensional volume" in the abbrev).
A unit n-ball has a volume of $$formula$$ which rapidly approaches zero as $$D$$ grows.
...
We can thus consider the sphere loosing some volume when a dimension is added.
This *volume loss* occurs in every additional dimension.
In a sense it has more dimensions to split it's unit radius amongst.
$$
\sqrt{d_1^2 + d_2^2 + \dots + d_n^2 } = 1
$$
The left hand side has an increasing number of terms while the right hand side remains constant.
