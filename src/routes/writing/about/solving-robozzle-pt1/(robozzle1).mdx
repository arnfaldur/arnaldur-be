...
A GA requires a fitness function that evaluates candidates[^1] and gives them a score.
...
The fitness function valued collecting stars highly and included a couple of heuristics to hopefully guide the search.
For example, moving to a heretofore untouched tiles was rewarded while each move had a small penalty to guide towards shorter solutions.
...
With the help of a crude multithreading method[^2] and a lot of optimizations I managed to solve puzzle 9 but there was no way it would solve problem 10.
...
This method could only find such a solution by randomly guessing it from scratch or through a very lucky crossover and mutation, which is likely not better than a brute force solver.

[^1]: A solution is a candidate that makes the robot get all the stars. A candidate is a program that consists of one or more functions, each containing nil or more symbols.
[^2]: I simply ran multiple instances of the solver manually and monitored the output of each. The solver worked by randomly sampling the solution space so conflicts and repeated work was not a big concern.
