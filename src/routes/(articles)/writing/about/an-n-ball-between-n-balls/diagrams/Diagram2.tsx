...
const diagram3D2 = (
	...
	createEffect(() => {
		...
		thirdCircleGroup.children.forEach((circle) => {
			circle.rotation.y = rads1;
			circle.position.x = (2 * Math.SQRT2 * cos(rads1)) / sec(rads1Inv) - 1;
			circle.position.z = 1 - (2 * Math.SQRT2 * sin(rads1)) / sec(rads1Inv);
			circle.scale.setScalar(
	...
	createEffect(() => {
		...
		const secTrigScale3 = Math.sqrt(1 + Math.pow(animCenter, 2)) * Math.cos(rads3);
